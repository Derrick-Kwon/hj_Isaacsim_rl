...
def overwrite_python_analysis_extra_paths(isaaclab_settings: str) -> str:
    ...
    # 1) isaac-sim 경로 기준으로 sibling인 IsaacLab/source 절대경로 계산
    isaaclab_root = os.path.normpath(
        os.path.join(ISAACSIM_DIR, "..", "IsaacLab", "source")
    )

    # 2) 디렉터리인 경우에만 하위 폴더들을 extraPaths에 추가
    if os.path.isdir(isaaclab_root):
        for ext in os.listdir(isaaclab_root):
            # 중복 방지: 이미 있으면 추가하지 않기
            rel_path = os.path.relpath(ISAACSIM_DIR, PROJECT_DIR)
            rel = f'"${{workspaceFolder}}/{rel_path}/../IsaacLab/source/{ext}"'
            if rel not in path_names:
                path_names.append(rel)
    else:
        print(f"IsaacLab/source is not found: {isaaclab_root}")
